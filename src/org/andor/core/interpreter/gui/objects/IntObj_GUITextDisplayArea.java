...
public class IntObj_GUITextDisplayArea extends MLInterpreterObject {

	/* The constructor */
	public IntObj_GUITextDisplayArea() {
		//Call the super constructor
		super("GUITextDisplayArea");
	}
	...
	public MLInterpretedObject interpret(MLObject currentObject, List<MLInterpretedObject> interpretedObjects) {
		...
		for (int i = 0; i < currentObject.getParameters().size(); i++) {
			...
			else if (currentObject.getParameter(i).getName().equals("width"))
				width = currentObject.getParameter(i).getFloatValue();
		}
		//Create the object
		GUITextDisplayArea displayArea = new GUITextDisplayArea(text, width);
		//Setup the rest of the component
		IntObj_GUIComponent.interpret(currentObject, interpretedObjects, displayArea);
		//Recalculate the text
		displayArea.calculateText(text);
		//Return the object
		return new MLInterpretedObject(currentObject.getName(), displayArea);
	}
	
	/* The method used to write a component */
	public void write(MLObject object, GUIComponent component) {
		//Add the parameters
		object.add(new MLParameter("text", ((GUITextDisplayArea) component).getText()));
