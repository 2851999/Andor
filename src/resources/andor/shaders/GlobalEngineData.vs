...
void andor_general_setup() {
	...
	frag_andor_normal = normalize(andor_modelMatrix * vec4(andor_normal, 0.0)).xyz;
	frag_andor_tangent = normalize(andor_modelMatrix * vec4(andor_tangent, 0.0)).xyz;
	...
	frag_andor_worldPosition = (andor_modelMatrix * vec4(andor_vertex, 1.0)).xyz;
	//Assign the position
